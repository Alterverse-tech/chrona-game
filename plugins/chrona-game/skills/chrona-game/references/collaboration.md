...
A standalone `index.html` over 4 MiB exceeds the text-entry limit even when it is otherwise a working static game. This technical cap remains in force. If the unpacked single-file HTML is no larger than 100 MB, a disclosed delivery-specific wrapper may be introduced to cross the cap without separate packaging approval, provided it keeps the original outside the wrapper, preserves gameplay, and verifies that recovered bytes match by SHA-256. If the unpacked single-file HTML is over 100 MB, preserve the original and report the incompatibility; explicit user authorization is required before introducing a wrapper. Test hosted behavior independently in either case. Prefer a project’s existing source/build split or a supported platform import route whenever available.
